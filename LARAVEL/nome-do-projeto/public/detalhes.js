(function () {
  ...
  function carregarProcesso() {
    ...
    const origemDados = obterOrigemProcessos();
    if (!origemDados) {
      mostrarErro('Não foi possível localizar a origem dos processos.');
      return;
    }

    const url = new URL(origemDados, window.location.origin);
