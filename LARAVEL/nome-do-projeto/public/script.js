(function () {
  ...
  function carregarProcessos() {
    const origem = obterOrigemProcessos();
    if (!origem) {
      exibirMensagemErro('A origem dos processos não está configurada.');
      return;
    }

    const url = new URL(origem, window.location.origin);
    ...
    fetch(url.toString(), { cache: 'no-store' })
      ...
      .catch(function (erro) {
        console.error('Falha ao obter processos a partir da API.', erro);
        exibirMensagemErro('Não foi possível carregar os processos. Verifique a conexão com o banco de dados.');
      });
  }

  function obterOrigemProcessos() {
    if (typeof document === 'undefined' || !document.body) {
      return null;
    }
    const origem = document.body.getAttribute('data-processes-url');
    if (!origem) {
      return null;
    }
