(function () {
  ...
  function armazenarProcessoParaDetalhes(processo) {
    const payload = JSON.stringify(processo);
    try {
      sessionStorage.setItem(DETALHE_STORAGE_KEY, payload);
    } catch (error) {
      console.warn('Não foi possível armazenar o processo para detalhes', error);
    }

    try {
      localStorage.setItem(DETALHE_STORAGE_KEY, payload);
    } catch (error) {
      console.warn('Não foi possível manter o processo no armazenamento local', error);
    }
  }
